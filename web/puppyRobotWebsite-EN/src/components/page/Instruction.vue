                        ...
                        <div v-for="(menu, index) in menuList" :key="menu.name">
                            ...
                            <div class="menuword">
                                ...
                                <li :class="(menu.nid == currDisplayNid || (menu.name == currTopLevelName)) ? 'topMenu_li currMenu':'topMenu_li'" @click="(menu.nid && !menu.has_child)?displayCon(menu, true):displaySubMenu(menu)">
                                    <div :class="isMoreThanMax(menu.name)?'topmenuicon topmenuicon_plus':'topmenuicon'">
                                        <img :src="require('./../../assets/images/support/icon_' + index + '.png')" >
                                    </div>
                                    <div :class="isMoreThanMax(menu.name)?'name_container line_height_half':'name_container'" :title="menu.name">{{menu.name}}</div>
 

                                    <img v-if="menu.has_child" v-show="menu.name == currTopLevelName && topLevelMenuExpand" :class="isMoreThanMax(menu.name)?'arrow arrow_plus':'arrow'" src="./../../assets/images/support/arrow_up.png">
                                    <img v-if="menu.has_child" v-show="menu.name != currTopLevelName || !topLevelMenuExpand" :class="isMoreThanMax(menu.name)?'arrow arrow_plus':'arrow'" src="./../../assets/images/support/arrow_down.png">
                                    ...
                                    <ul class="subMenu_ul" v-show="menu.name == currTopLevelName && topLevelMenuExpand">
                                        <div v-for="(subMenu, index) in menu.list" :key="subMenu.name">
                                            <li :id="'support?id=' + subMenu.nid" @click.stop :class="subMenu.nid == currDisplayNid ? 'subMenu_li currSubMenu':'subMenu_li'" :style="index==0?'border-top: 1px solid #f2f2f2;':''" @click="displayCon(subMenu, false)">
                                                <div :class="isMoreThanMax(subMenu.title)?'subtitle_container line_height_half':'subtitle_container'">{{subMenu.title}}</div>
                                            </li>
                                        </div>
                                    </ul>
                                </li>
...
export default {
    ...
    mounted() {
        ...
        this.$store.dispatch('getInstructionMenu').then((result)=> { 
            ...
            if(this.menuList) {
                var tnid = this.GetQueryString('id')? this.GetQueryString('id'): '120';
                var ttitle = tnid == '120'? 'About' : '';
                if(tnid) {
                    var tempMenu = {
                        nid: tnid,
                        title: ttitle,
