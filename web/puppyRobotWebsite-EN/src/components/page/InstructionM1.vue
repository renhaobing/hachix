<template>
    <div id="instruction" class="clear">
        <div :class="['btm', {'en':isEn}]">
            <div class="left">
                <ul class="topLevelMenu_ul">
                    <li v-for="(menu) in menuList" :key="menu.name" :id="'id=' + menu.id" :class="menu.id == currTopLevelId? 'topMenu_li currMenu':'topMenu_li'" @click="(menu.id && !menu.children)?displayCon(menu, 1):displaySubMenu(menu, 1)">
                        <div class="topmenu">
                        ...
                        <ul class="subMenu_ul" v-if="menu.type != 1" v-show="menu.Expand && menu.id == currTopLevelId">
                            <li v-for="(subMenu, index2) in menu.children" :key="subMenu.id" :id="'subMenuid=' + subMenu.id" :class="subMenu.id == currLevel2Id ? 'subMenu_li currSubMenu':'subMenu_li'" @click.stop="(subMenu.id && !subMenu.children)?displayCon(subMenu, 2):displaySubMenu(subMenu, 2)">
                                <template v-if="subMenu.children && subMenu.id !=1">
                                    <p :class="(subMenu.id == currLevel2Id && subMenu.Expand)?'up_arrow':'down_arrow'">{{subMenu.name}}</p>
...
export default {
    ...
    mounted() {
        ...
        this.$store.state.menuId = 2;//this.$route.query.id?this.$route.query.id:2;
        // this.$route.query && this.$route.query.id == 2?this.isEn=true:this.isEn=false;
        this.$store.dispatch('getInstructionMenuM1').then((result)=> { 
    ...
    methods: {
        displayCon: function(menu, menuLevel) {
            ...
            this.$store.dispatch('getInstructionM1').then((result)=> { 
