    ...
    <div id="instruction">
                                ...
                                <li :class="(menu.nid == currDisplayNid || (menu.name == currTopLevelName)) ? 'topMenu_li currMenu':'topMenu_li'" @click="(menu.nid && !menu.has_child)?displayCon(menu, true):displaySubMenu(menu)">
                                    ...
                                    <ul class="subMenu_ul" v-show="menu.name == currTopLevelName && topLevelMenuExpand">
                                        <div v-for="(subMenu, index) in menu.list" :key="subMenu.name">
                                            <li :id="'support?id=' + subMenu.nid" @click.stop :class="subMenu.nid == currDisplayNid ? 'subMenu_li currSubMenu':'subMenu_li'" :style="index==0?'border-top: 1px solid #f2f2f2;':'', isMoreThanMax(subMenu.title, 36)?'line-height: 22px; padding-top: 5px; padding-bottom: 5px;':''" @click="displayCon(subMenu, false)">
                                                {{subMenu.title}}
                                            </li>
                                        </div>
                                    </ul>
        ...
        <div class="right">
            <div class="content_title">
                <div :class="isMoreThanMax(currDisplayName, 27)?'title_small':'title'">{{currDisplayName}}</div>
...
export default {
    ...
    methods: {
        ...
        isMoreThanMax: function(name, maxCount) {
